...
private:
  ...
  const QDir m_bitcoinoteDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfBitcoinoteBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _bitcoinote) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo bitcoinoteBlockchainInfo() const;
