...
const quint32 BITCOINOTE_BLOCK_SIZE = 0xd5;
...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_bitcoinoteDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfBitcoinoteBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 bitcoinoteHeight;
  if (!checkIfBlockchainOutdated(currentHeight, bitcoinoteHeight)) {
    return;
  }
  ...
  QuestionDialog dlg(tr("Blockchain installation"), QString(questionStringTemplate).arg(bitcoinoteHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::getGenesisBlockFromBlockchain(char** _genesisBlockData) const {
  ...
  if (blockSize != BITCOINOTE_BLOCK_SIZE) {
    return false;
  }
...
bool BlockchainInstaller::checkIfBitcoinoteBlockchainExists() const {
  return m_bitcoinoteDir.exists() && m_bitcoinoteDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_bitcoinoteDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::bitcoinoteBlockchainInfo() const {
  return QFileInfo(m_bitcoinoteDir.absoluteFilePath(m_blocksFileName));
}
...
void BlockchainInstaller::installBlockchain() {
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_bitcoinoteDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_bitcoinoteDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
