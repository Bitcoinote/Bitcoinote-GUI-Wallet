...
const char BITCOINOTE_URI_SCHEME_NAME[] = "bitcoinote";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("Bitcoinotewallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("Bitcoinotewallet.lock"));
  QUrl paymentUrl = QUrl::fromUserInput(arguments().last());
  if (paymentUrl.scheme() != BITCOINOTE_URI_SCHEME_NAME) {
    paymentUrl = QUrl();
  }
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] Bitcoinote wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "Bitcoinote wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have Bitcoinoted with non-default RPC port, you should terminate it and relaunch BitcoinoteWallet\n"
      "or\n"
      "Set the Local deamon required port in BitcoinoteWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
