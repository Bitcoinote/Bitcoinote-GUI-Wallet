...
//
// Bitcoinote is free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Bitcoinote is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Lesser General Public License for more details.
//
// You should have received a copy of the GNU Lesser General Public License
// along with Bitcoinote.  If not, see <http://www.gnu.org/licenses/>.
...
const char DEFAULT_WALLET_FILE_NAME[] = "bitcoinotewallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.bitcoinote.org:7707";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("bitcoinotewallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("bitcoinotewallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("bitcoinotewallet.desktop");
  ...
  res = autorunSettings.contains("BitcoinoteWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("BitcoinoteWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.bitcoinote.bitcoinotewallet");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("BitcoinoteWallet", appPath);
    } else {
      autorunSettings.remove("BitcoinoteWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\bitcoinote", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:bitcoinote");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\bitcoinote\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\bitcoinote\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("bitcoinotewallet.cfg"));
